...
function BranchesLandingPage({ BranchDetails }) {
  ...
  const [selectedImg, setSelectedImg] = useState(
    branchData.branchImage ? branchData.branchImage : "/images/Frame 22.png"
  );
  const corosalImgs = [
  ...
  const gallery =
    BranchDetails?.data?.branchGallery.length > 0
  ...
  return (
    <MaxWebWidth
      sectionStyling="bg-coursegradient "
      articalStyling="flex justify-between mobile:flex-col"
    >
        ...
        <article className="flex gap-2.5 sm:pt-[3.333vh] mobile:pb-[1.502vh]">
          ...
          <p className=" text-[1.25vw] mobile:text-[2.791vw] pl-[0.625vw]">
            ...
            <div className="flex flex-col items-end">
              <button
                onClick={() =>
                  window.open(branchData?.address.location, "_blank")
                }
                className="flex gap-1  text z-1"
              >
                <p className="text-orange-500 text-[0.703vw] p-[0.156vw]">
                  Get Directions
                </p>
                <Svg
