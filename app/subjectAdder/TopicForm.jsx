...
const TopicForm = ({ initialValues, onUpdateTopic }) => {
  ...
  const handleAddSubTopic = () => {
    // formik.handleSubmit()
    formik.values.topicTitle && setSubTopics([
      ...subTopics,
      { subTopicTitle: '', subTopicPreviewUrl: '', subTopicPreviewDuration: '' },
    ]);
  };
    ...
    <div className='m-auto border border-gray-300 p-8 mt-4 mb-4 rounded-xl'>
        ...
        <div >
          <form onSubmit={formik.handleSubmit}>
            <div>
              <label><span className="text-red-500 pr-1 ">*</span>Topic Name</label>
              <Input
                type="text"
                name="topicTitle"
              ...
              />
              {formik.errors.topicTitle ? <div className='text-red-500 text-[0.75vw]'>{formik.errors.topicTitle}</div> : null}
            </div>
            <div>
              <label>URL</label>
              <Input
              ...
              />
              {formik.errors.topicPreviewUrl ? <div>{formik.errors.topicPreviewUrl}</div> : null}
            </div>
            <div>
              ...
              />
              {formik.errors.topicPreviewDuration ? <div>{formik.errors.topicPreviewDuration}</div> : null}
            </div>
            {subTopics.map((subTopic, index) => (
          <SubTopicForm
            key={index}
            initialValues={subTopic}
            onUpdateSubTopic={(updatedSubTopic) => handleUpdateSubTopic(updatedSubTopic, index)}
          />
        ))}
            <button type="button" className="py-2 mt-3 px-4 bg-gradient rounded-md text-white" onClick={handleAddSubTopic}>
          Add SubTopic
        </button>
