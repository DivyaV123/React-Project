...
const CollegeFilter = () => {
    ...
    const { data: collegeData, refetch } = useGetAllCollegesQuery(universitySelected);
    const collegeList = collegeData?.response.filter(college => college !== "");
    useEffect(() => {
        refetch();
    }, [universitySelected]);
    ...
    const renderCheckbox = (item, index) => (
        <Checkbox
            ...
            onChange={() =>
                handleCommonFilter(index, selectedCity, setSelectedCity, collegeList, "college")
            }
        />
    );
