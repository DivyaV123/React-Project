...
function SubjectMapping() {
    ...
    const [subjectMapping, { isLoading: mapSubjectLoading }] = useSubjectMappingMutation();
    ...
    const formikDetails = useFormik({
        ...
        onSubmit: async (values, { resetForm }) => {
            ...
            selectedSubjectID.map((ele) => {
                console.log(ele,{selectedSubjectID},{selectedCategoryId},{allId})
                payload.push(ele.Id)
            })
            if (selectedSubjectID) {
                try {
                   
                    const response = await subjectMapping({ payload: payload, courseId: allId.courseId }).unwrap();
                    console.log({payload},{response})
                    if(response.status === 200 ){
                        toast({
                            variant: "success",
                            title: (
                              <span className=" font-bold  "> Subject Mapped Successfully</span>
                            ),
                          });
                          setTimeout(() => {
                    ...
                    toast({
                        variant:"destructive",
                        title: (
                          <span className=" font-bold  "> {err?.data?.data}</span>
                        ),
                      });
                }
            } 
            ...
            <MaxWebWidth sectionStyling='p-8' articalStyling='border border-gray-300 p-8 rounded-xl'>
                <div>
                    <form onSubmit={formikDetails.handleSubmit}>
                        ...
                        <p className='text-slate-400 p-1'>Map subject for the selected Course</p>
