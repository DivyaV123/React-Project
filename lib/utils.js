...
export function filterOnlineCourses(data) {
  return data.map(course => {
    const filteredCourseResponse = course?.courseResponse?.filter(cr =>
      cr?.modes?.includes("ONLINE_CLASSES")
    );
    const filteredSubCourses = course?.subCourse?.map(subCourse => {
      const filteredSubCourseResponse = subCourse?.subCourseResponse?.filter(scr =>
        scr?.modes?.includes("ONLINE_CLASSES")
      );

      return {
        ...subCourse,
        subCourseResponse: filteredSubCourseResponse,
      };
    }).filter(subCourse => subCourse?.subCourseResponse?.length > 0);


    if (filteredCourseResponse?.length > 0 || filteredSubCourses?.length > 0) {
      return {
        ...course,
...
export function slugifyURL(title) {
  return title
    ?.toString()
    .toLowerCase()
    .trim() 
